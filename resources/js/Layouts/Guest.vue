        ...
        <nav class="flex items-center justify-between">
            ...
            <div v-if="canLogin" class="flex items-center space-x-12">
                <Link v-if="$page.props.auth.user" :href="route('dashboard')" :active="route().current('dashboard')"
                    class="rounded border border-gray-900 px-8 py-2 text-base font-medium">Dashboard</Link>

                <div v-else class="flex items-center space-x-12">
                    <Link :href="route('login')" :active="route().current('login')" class="text-base font-medium">Sign
                    In</Link>

                    <Link v-if="canRegister" :href="route('register')" :active="route().current('register')"
                        class="rounded border border-gray-900 px-8 py-2 text-base font-medium">Sign Up</Link>
                </div>
            </div>
        </nav>
